...
+ (void)saveStreamData:(id)responseObject {
    ...
    if (responseObject[@"shows"]) {
        ...
        for (NSDictionary *streamentry in shows) {
            ...
            NSString *sitesjson = [[NSString alloc] initWithData:[NSJSONSerialization dataWithJSONObject:streamentry[@"sites"] options:NSJSONWritingSortedKeys error:nil] encoding:NSUTF8StringEncoding];
